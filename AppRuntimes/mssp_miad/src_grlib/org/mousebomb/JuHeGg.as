...
public class JuHeGg extends EventDispatcher
{
	...
	private var _defMiAppID:String = "";
	private var _defMiSplashId:String = "";
	private var _defMiInterstitialId:String = "";
	 ...
	 * @param app_id 应用ID。
	 */
	public function init( app_id:int, baiduAppID:String,baiduBanner:String,baiduInterstitial:String, miAppID:String,miSplashID:String, miInterstitialID:String ):void
	{
		DebugHelper.log("初始化AD,AppID:"+app_id);
		DebugHelper.log("预设:BAIDU:"+baiduAppID+","+baiduBanner+","+baiduInterstitial +";miSplashID:"+miSplashID + ",miInterstitialID="+miInterstitialID);
		initANEs();
		...
		_defMiAppID = miAppID;
		_defMiSplashId = miSplashID;
		_defMiInterstitialId = miInterstitialID;
	...
	public function showBanner( horizontal:String = CENTER, vertical:String = TOP ):void
	{
		...
		hideBanner();
		if(_baiduBannerPercent==0) return;
...
//			_baidu.showBanner( BaiDu.BANNER, RelationPosition.BOTTOM_CENTER );
	...
	public function showInterstitial():void
	{
		...
		if(isBaidu){
			...
			} else if( _miAd.isInterstitialReady()  && _admobInterstitialPercent>0 )
			{
				_miAd.showInterstitial();
				hideBanner();
				DebugHelper.log("baiduNotReady,_miAd.showInterstitial()");
			}else{
				showSha();
			}
		}else{
			if( _miAd.isInterstitialReady())
			{
				_miAd.showInterstitial();
				hideBanner();
				DebugHelper.log("_miAd.showInterstitial()");
	...
	public function cacheInterstitial():void
	{
		...
		if(!_miAd.isInterstitialReady() && _admobInterstitialPercent>0)
		{
			_miAd.cacheInterstitial();
			DebugHelper.log("_miAd.cacheInterstitial()");
		}
	}
}
}
