package tiezhi
{
	...
	public class TZGame extends Sprite implements IDispose,IFlyIn
	{
		...
		public function changeLevel(level : int) : void
		{
			...
			//
			levelModel = LevelModel.getInstance();
			nextI = (levelModel.level-1) * LevelModel.NEWBIRDSCOUNT_INLEVEL;
...
private var _shine:MovieClip;
		...
		private function userCorrect(dragHoleLi : HoleLi, shelfHoleLi : HoleLi) : void
		{
			...
			if (--targetsLeft < 1)
			{
				...
				// 一关显示成语读音拼音
				ui.win.nameTf.text = ChengYuTimu.getLevelName(levelModel.level);
				ui.win.descTf.text = ChengYuTimu.getLevelDesc(levelModel.level);
				ui.win.visible = true;
				...
				SoundMan.playChengYu(levelModel.level);
...
//		private static const HOLE_SHELF_TOP : Number = 180;
		...
		{
			var picI:int=((nextI++) % GameConf.PIC_NUM) + 1;
			newHoleLiFromPicI( picI, HoleLi.DRAG_TARGET, li);
		}
