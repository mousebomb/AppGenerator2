...
package com.shortybmc.data.parser
{
	...
	public class CSV
	{
		...
		public function getField(key:String, bodyId:*):*
		{
			var id:int;
			if(bodyId is Array)
				id = getData().indexOf(bodyId);
			else if(bodyId is int) id = bodyId;
			return getRecordSet(id)[header.indexOf(key)];
		}
		 ...
		 * @return
		 */
		public function parsePoints(field :String ) : Vector.<Point>
		{
			var end : Vector.<Point> = new <Point>[];
			var arr :Array = field.split("|");
			for(var i:int = 0;i<arr.length;i++)
			{
				var li : String = arr[i];
				var kv :Array= li.split("-");
				end.push(new Point(kv[0],kv[1]));
			}
			return end;
		}
