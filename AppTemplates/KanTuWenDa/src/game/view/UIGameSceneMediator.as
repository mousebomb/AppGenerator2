package game.view {
	...
	public class UIGameSceneMediator extends Mediator
	{
		...
		override public function onRegister() : void
		{
            ...
            questionPicContainer = new Sprite();
            questionPicContainer.x = 18;
            questionPicContainer.y = 18;
            ui.qp.addChild(questionPicContainer);
         ...
         * 承载问题的容器
         */
        private var questionPicContainer :Sprite;
		/**
		 * 题目更新
		 */
		private function onQuestionChange(e : LevelEvent) : void
		{
            ...
            for(var i : int = questionPicContainer.numChildren-1; i>=0;i--)
                questionPicContainer.removeChildAt(i);
            var hasClass :Boolean = ApplicationDomain.currentDomain.hasDefinition(q.question);
            if(hasClass)
            {
                var clazz : Class = getDefinitionByName(q.question) as Class;
                questionPicContainer.addChild(new clazz());
            }else{
                throw new Error("找不到导出的元件:" + q.question);
            }
		}
