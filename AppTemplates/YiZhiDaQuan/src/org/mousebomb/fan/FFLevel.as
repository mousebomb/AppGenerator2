package org.mousebomb.fan
{
	 ...
	 * @author rhett
	 */
	public class FFLevel extends Sprite  implements IDispose,IFlyIn
	{
		private var ui : UITZLevel;

		public function FFLevel()
		{
			ui = new UITZLevel();
			addChild(ui);
			...
			ui.addChild(shelf);
			//
			var levelModel : FFLevelModel = FFLevelModel.getInstance();
			levelModel.initAllLevels();
			shelf.setList(levelModel.ffLevels);
		...
		private function onAddLi(li : NumLevelBtn, vo : FFLevelVO) : void
		{
		...
		private function onLiUp(event : MouseEvent) : void
		{
			...
			var level :int =  (li.vo as FFLevelVO).level;
			FFLevelModel.getInstance().level = level;
			YiZhiDaQuan.instance.replaceScene( new FFGame() );
		}
