package org.mousebomb.tiezhi
{
	...
	public class TZLevel extends Sprite  implements IDispose,IFlyIn
	{
		...
		public function TZLevel()
		{
			...
			ui.addChild(shelf);
			//
			var levelModel : TZLevelModel = TZLevelModel.getInstance();
			levelModel.initAllLevels();
			shelf.setList(levelModel.tzLevels);
		...
		private function onAddLi(li : NumLevelBtn, vo : TZLevelVO) : void
		{
		...
		private function onLiUp(event : MouseEvent) : void
		{
			...
			var level :int =  (li.vo as TZLevelVO).level;
			TZLevelModel.getInstance().level = level;
			YiZhiDaQuan.instance.replaceScene( new TZGame() );
		}
