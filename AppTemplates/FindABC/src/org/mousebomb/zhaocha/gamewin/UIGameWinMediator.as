package org.mousebomb.zhaocha.gamewin
{
	...
	public class UIGameWinMediator extends Mediator
	{
		...
		override public function onRegister() : void
		{
			...
			ui.x = GameConf.DESIGN_SIZE_W * .5;
			ui.y = GameConf.VISIBLE_SIZE_H * .5;
			ui.scaleX = 2;
			ui.scaleY = 2;
			TweenLite.to(ui, 1.0, {scaleX:1.0, scaleY:1.0, ease:Back.easeInOut, onComplete:tweenComp});
		}

		private function tweenComp() : void
		{
			(contextView as Sprite).addEventListener(MouseEvent.CLICK, onClick);
		}

		private function onClick(event : MouseEvent) : void
		{
			(contextView as Sprite).removeEventListener(MouseEvent.CLICK, onClick);
